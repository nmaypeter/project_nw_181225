                ...
                for prod_setting in [1, 2]:
                    for prod_setting2 in [1, 2, 3]:
                        ...
                        for bud in [1, 5, 10]:
                            ...
                            ssr_sample = SeedSelectionR(graph_dict, seed_cost_dict, product_list, bud, pps, wpiwp)
                            ...
                            notban_set = ssr_sample.updateNotbanSet(notban_set, 0)
                            ...
                            mep_k_prod, mep_i_node = ssr_sample.selectRandomSeed(nban_set)
                            ...
                            while now_budget < bud and mep_i_node != '-1':
                                ...
                                nban_set = ssr_sample.updateNotbanSet(nban_set, now_budget)
                                mep_k_prod, mep_i_node = ssr_sample.selectRandomSeed(nban_set)

                            # -- result --
                            how_long = round(time.time() - start_time, 2)
                            fw = open("result/samples/mric_pps" + str(pps) + "_wpiwp" * wpiwp + "_" + data_set_name + "_" + product_name + "_b" + str(bud) + ".txt", 'w')
